        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="form-card">
                ...
                <form action="{{ route('formation.inscription') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="formation" id="formation" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            ...
                            @foreach ($formations as $formation)
                                <option value="{{ $formation->id }}" {{ old('formation') == $formation->id ? 'selected' : '' }}>{{ $formation->nom }} - {{ $formation->duree }}</option>
                            @endforeach
                        </select>
                        @error('formation')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div class="mb-4">
                        <label for="acte_naissance_path" class="block text-gray-700 font-medium">Acte de naissance</label>
                        <input type="file" id="acte_naissance_path" name="acte_naissance_path" class="w-full border-gray-300 rounded-lg shadow-sm" required>
                    ...
                    <div class="mb-4">
                        <label for="cni_path" class="block text-gray-700 font-medium">Carte Nationale d'Identité (CNI)</label>
                        <input type="file" id="cni_path" name="cni_path" class="w-full border-gray-300 rounded-lg shadow-sm" required>
                        @error('cni_path')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        <label for="diplome_path" class="block text-gray-700 font-medium">Diplôme</label>
                        <input type="file" id="diplome_path" name="diplome_path" class="w-full border-gray-300 rounded-lg shadow-sm" required accept=".pdf,.jpg,.jpeg,.png">
                        @error('diplome_path')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        <label for="autres_documents_paths" class="block text-gray-700 font-medium">Autres documents (optionnel)</label>
                        <input type="file" id="autres_documents_paths" name="autres_documents_paths[]" class="w-full border-gray-300 rounded-lg shadow-sm" multiple>
